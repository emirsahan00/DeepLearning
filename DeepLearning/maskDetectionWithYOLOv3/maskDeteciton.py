import cv2 #gerekli paketleri(kütüphaneleri tanımlıyoruz)
import numpy as np

cap = cv2.VideoCapture(0)  #görüntüyü web camden alıyoruz
...
while True: #frameleri okumak için sonsuz bir döngü başlatıyoruz
    ...
    labels = ["Mask","None","Without Mask"]

    colors = ["0,255,0","0,0,255"] #hangi renklerde bounding box olmasını istiyorsak o rengin bgr karşılığını giriyoruz
    colors = [np.array(color.split(',')).astype('int') for color in colors]
    colors = np.array(colors)
    colors = np.tile(colors,(18,1)) #diziyi büyütüyoruz

    model = cv2.dnn.readNetFromDarknet("D:/ImageProcessing/YOLOv3/yolov3_mask.cfg","D:/ImageProcessing/YOLOv3/yolov3_mask_last.weights") #modelimizi içe aktardık
    ...
    for max_id in max_ids:
        ...
        cv2.rectangle(frame,(start_x,start_y),(end_x,end_y),box_color,2) #nesnemizin çevreleyen bir dikdörtgen çiziyoruz(bounding box)
